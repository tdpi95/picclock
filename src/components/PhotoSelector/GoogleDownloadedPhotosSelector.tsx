...
export function GoogleDownloadedPhotosSelector({
    ...
    const [photos, setPhotos] = useState<{ id: string; thumbnail: string; original: string }[]>([]);
        ...
        try {
            ...
            const photoData = await Promise.all(
                keys.map(async (key) => {
                    ...
                    return { id: key, thumbnail: thumb || "", original: original || "" };
                })
            );
            setPhotos(photoData);
        } catch (err) {
            ...
            <div className="p-6 h-[60vh] overflow-y-auto">
                        ...
                        {photos.map((photo) => (
                            ...
                            >
                                ...
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                                    <button
                                        onClick={() => window.open(photo.original, "_blank")}
                ...
                {photos.length === 0 && !loading && (
                    <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
                        ...
                        <p className="text-sm">Use the "Select Photos" button to pick photos from Google.</p>
                    </div>
                )}
            </div>
        </Dialog>
